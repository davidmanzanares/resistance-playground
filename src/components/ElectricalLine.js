...
export default function ElectricalLine(props){
    ...
    const points = props.points.map(p => new Vector3(...p));
    const lines = [];
    ...
    return (
      <group>
      <group onPointerMove={ev => {hover(ev.point)}} onPointerLeave={ev=> hover(new Vector3(10000,0,0))}>
        {lines}
      </group>
        <Points
